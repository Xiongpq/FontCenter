...
SWFUpload.prototype.getFlashHTML = function (loadFlash) {
	return ['<object id="', this.movieName, '" type="application/x-shockwave-flash"', loadFlash ? 'data="' + this.settings.flash_url + '"' : '', 'width="', this.settings.button_width, '" height="', this.settings.button_height, '" class="swfupload">', '<param name="wmode" value="', this.settings.button_window_mode, '" />', '<param name="movie"', loadFlash ? 'value="' + this.settings.flash_url + '"' : '', '/>', '<param name="quality" value="high" />', '<param name="menu" value="false" />', '<param name="allowScriptAccess" value="always" />', '<param name="flashvars" value="' + this.getFlashVars() + '" />', "</object>"].join("")
};
...
SWFUpload.prototype.buildParamString = function () {
	...
	if (window.Identity) {
		c.AUTHID = Identity.AUTHID;
		c.ASPSESSID = Identity.ASPSESSID;
	}
	var b = [];
	if (typeof (c) === "object") {
		for (var a in c) {
...
function Uploadify($) {
		...
		init: function (options, swfUploadOptions) {
			...
			return this.each(function () {
				...
				var settings = $.extend({
					...
					fileSizeLimit: 102400, // The maximum size of an uploadable file in KB (Accepts units B KB MB GB if string, 0 for no limit)
					...
					hint: '允许批量上传任意格式的文件(单个文件小于100兆，空文件除外)',
				...
				if (flashInstalled) {
					...
					var movie = $('#' + swfuploadify.movieName);
					...
					movie.replaceWith($(window['uploadify_' + settings.id].getFlashHTML(true)).css({
						'position': 'absolute', 'z-index': 9999, 'left': 0
					}));
					...
					// Append the uploader hint
					if ($wrapper.find('.uploadify-limitation-hint').length == 0) {
						$wrapper.after($('<span class="uploadify-limitation-hint">' + settings.hint + '</span>'));
					}
				...
				setTimeout(function () {
					Invisible($wrapper.parent(), false);
				}, 300);
	...
	var handlers = {
		...
		onSelect: function (file) {
			...
			//var invalidChar = /[\\\/\:\*\?\"\<\>\|\#\{\}\%\~\&]/;
			//if (invalidChar.test(file.name)) {
			//	alert('文件名不能包含以下任何字符: \ / : * ? " < > | # { } % ~ & ' + '\n\n以下文件无法上传:\n\n' + file.name);
			//	this.cancelUpload(file.id);
			//	this.queue.filesCancelled++;
			//	return false;
			//}
			...
			if (settings.itemTemplate == false) {
				settings.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item">\
					...
					<span class="fileName inline-block ellipsis-content new" title="${fileName}">${fileName}</span><span>(${fileSize})</span><span class="data"></span>\
				...
				switch (errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							this.queueData.errorMsg += '\n\n选择的文件数量超过了限制 (' + errorMsg + ').';
						} else {
							this.queueData.errorMsg += '\n\n选择的文件数量超出了上传队列的容量 (' + settings.queueSizeLimit + ').';
						}
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 超过了文件大小限制 (' + settings.fileSizeLimit + ' KB)。';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 是空文件。';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 的文件类型不允许被上传 (' + swfuploadify.settings.fileTypeDesc + ')。';
						break;
				}
			}
